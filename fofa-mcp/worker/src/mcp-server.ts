...
import { z } from "zod";
import { type FOFAConfig, isOfficialAPI, fofaSearch, formatResults, normalizeSearchOptions } from "./fofa-client.js";
...
export function createMcpServer(config: FOFAConfig): McpServer {
  ...
  server.tool(
    ...
    '使用 FOFA 搜索引擎查询网络资产。支持 FOFA 查询语法，如 domain="example.com"、ip="1.1.1.1"、title="login" 等。',
    {
      ...
      size: z.number().int().min(1).max(MAX_SIZE).optional().describe("每页数量，默认 50（免费用户上限 50），最大 10000"),
      full: z.boolean().optional().describe("是否搜索全部数据，默认 false"),
    },
    async ({ query, fields, page, size, full }) => {
      try {
        const f = fields || DEFAULT_FIELDS;
        const options = normalizeSearchOptions(config, size || DEFAULT_SIZE, full || false);
        const result = await fofaSearch(config, query, f, page || 1, options.size, options.full);
