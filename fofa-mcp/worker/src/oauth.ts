...
<div class="card">
  <h1>FOFA MCP 授权</h1>
    <p class="subtitle">可填写官方 API Key，或网页端 access token（免费用户每次最多 50 条）</p>
  <form method="POST" action="${serverUrl}/authorize">
    ...
    <p class="hint">网页 token 使用 https://api.fofa.info/v1；留空会自动选择</p>
    <label for="api_key">API Key</label>
    <input type="password" id="api_key" name="api_key">
    <label for="email">邮箱（可选）</label>
    <input type="text" id="email" name="email">
    <label for="web_token">网页端 access token（与 API Key 二选一）</label>
    <input type="password" id="web_token" name="web_token" placeholder="从网页请求 Authorization 复制">
...
export async function handleAuthorizePost(body: URLSearchParams, secrets: ServerSecrets, defaultWebPrivateKey = ""): Promise<Response> {
  const apiKey = body.get("api_key");
  const webToken = body.get("web_token")?.trim() || "";
  if (!apiKey && !webToken) {
    return new Response("缺少 api_key 或 web_token", { status: 400 });
  }

  const submittedBase = body.get("base_url")?.trim() || "";
  const baseURL = (webToken && (!submittedBase || submittedBase === "https://fofa.info")
    ? "https://api.fofa.info/v1" : (submittedBase || "https://fofa.info")).replace(/\/+$/, "");
  ...
  const code = await encryptCode(secrets, {
    ...
    webPrivateKey: defaultWebPrivateKey || body.get("web_private_key") || undefined,
...
export async function extractConfigFromToken(
...
): Promise<{ config: { apiKey: string; email: string; baseURL: string; webToken?: string; webPrivateKey?: string; userId: string; plan: "free" | "paid"; maxPageSize: number; allowFull: boolean }; clientId: string } | Response> {
  ...
  try {
    ...
    return {
      config: { apiKey: payload.apiKey, email: payload.email, baseURL: payload.baseURL, webToken: payload.webToken, webPrivateKey: payload.webPrivateKey, userId: payload.userId, plan: payload.plan, maxPageSize: payload.maxPageSize, allowFull: payload.allowFull },
      clientId: payload.clientId,
    };
  } catch {
    return Response.json(
