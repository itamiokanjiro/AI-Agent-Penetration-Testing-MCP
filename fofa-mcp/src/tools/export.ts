...
import { type FOFAConfig, normalizeSearchOptions, search } from "../client.js";
  ...
  server.tool(
    ...
    async ({ query, output_file, fields, page, size, full }) => {
      try {
        ...
        const p = page || 1;
        const requested = Math.min(size || DEFAULT_SIZE, MAX_SIZE);
        const options = normalizeSearchOptions(config, requested, full || false);
        const result = await search(config, query, f, p, options.size, options.full);
