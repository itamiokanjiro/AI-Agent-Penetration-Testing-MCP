...
export async function decryptCode(secrets: ServerSecrets, jwe: string): Promise<CodePayload> {
  ...
  return {
    ...
    webToken: payload.webToken as string | undefined,
    webPrivateKey: payload.webPrivateKey as string | undefined,
...
export async function verifyToken(secrets: ServerSecrets, jws: string): Promise<TokenPayload & { exp: number }> {
  ...
  return {
    ...
    plan: payload.plan === "paid" ? "paid" : "free",
    maxPageSize: Number(payload.maxPageSize) || 50,
    allowFull: payload.allowFull === true,
