...
<div class="card">
  ...
  <form method="POST" action="${params.serverUrl}/authorize">
    ...
    <label for="base_url">FOFA API 地址（可选）</label>
    <input type="text" id="base_url" name="base_url" value="https://fofa.info" placeholder="https://fofa.info 或 https://api.fofa.info/v1">
    <p class="hint">使用网页 token 时填写 https://api.fofa.info/v1；留空会自动选择</p>

    <label for="api_key">API Key</label>
    <input type="password" id="api_key" name="api_key" placeholder="你的 FOFA API Key">

    <label for="email">邮箱（官方 API 必填；中转 API 按其要求）</label>
